<div class="trace-line-header break-long-words <?= $trace['file'] ? 'sf-toggle' : ''; ?>" data-toggle-selector="#trace-html-<?= $prefix; ?>-<?= $i; ?>" data-toggle-initial="<?= 'expanded' === $style ? 'display' : ''; ?>">
    <?php if ($trace['file']) { ?>
        <span class="icon icon-close"><?= $this->include('{{url('')}}/public/assets/images/icon-minus-square.svg'); ?></span>
        <span class="icon icon-open"><?= $this->include('{{url('')}}/public/assets/images/icon-plus-square.svg'); ?></span>
    <?php } ?>
    ...
    <?php if ($trace['file']) { ?>
        ...
        <span class="block trace-file-path">
            ...
            <span class="icon icon-copy hidden" data-clipboard-text="<?php echo implode(\DIRECTORY_SEPARATOR, $filePathParts).':'.$lineNumber; ?>">
                <?php echo $this->include('{{url('')}}/public/assets/images/icon-copy.svg'); ?>
            </span>
        </span>
    <?php } ?>
</div>
